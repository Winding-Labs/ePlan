...
function PureMultimodalInput({
  ...
  // Use the new upload hook with 50MB default limit for chat attachments
  const { upload } = useFileUpload({
    maxSize: 50 * 1024 * 1024, // 50MB - reasonable for chat attachments
    onError: (error) => {
  ...
  const uploadFile = useCallback(
    async (file: File) => {
      try {
        ...
        return {
          url: result.url,
          // Display the user's filename, not the R2 storage pathname.
          name: file.name,
  ...
  // Removes the attachment from the outgoing message only; the uploaded file
  ...
  return (
    <div className="relative w-full flex flex-col gap-4">
      ...
      <Textarea
        ...
        className={cx(
          "min-h-[24px] max-h-[calc(75dvh)] overflow-hidden resize-none rounded-2xl !text-base border-input bg-muted pb-10 shadow-none dark:border-zinc-700",
          className,
        )}
        rows={2}
        autoFocus
