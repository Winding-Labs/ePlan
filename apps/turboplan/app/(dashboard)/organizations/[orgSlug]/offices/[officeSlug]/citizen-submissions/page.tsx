...
export async function generateMetadata({
  ...
  try {
    ...
    return {
      title: `Citizen Submissions - ${office.name} - ${organization.name}`,
      description: `View citizen submissions for ${office.name}`,
    };
  } catch (error) {
...
export default async function CitizenSubmissionsPage({
  params,
}: OfficePageProps) {
  const session = await getCachedSession();
  const resolvedParams = await params;
  ...
  // Citizens cannot access this page — redirect to the office page
