/**
 * Pure helpers for storing cataloger documents in public blob storage.
 * Kept free of I/O so they can be unit-tested without a DB or network.
 */
 ...
 */
export const resolveCatalogerDocumentMimeType = (
...
/**
 * Make a single safe path segment: no separators, no control characters, no
...
/**
 * Build a filename for a cataloger document from its source URL, falling back
 * to the document title. The result is always a single key segment (decoded
 * `%2F` / `%5C` cannot add path segments) and ends with the extension that
 * matches the stored MIME type.
 */
export const buildCatalogerDocumentFilename = (
  url: string,
  title: string,
  mimeType: string,
): string => {
