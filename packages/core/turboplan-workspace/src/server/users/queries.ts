...
import type { UserSearchEntityScope } from "./scope";
import type { UserSearchResult } from "./types";

/**
 * Who a search may fuzzy-match: members of one organization tree, or (admin
 * only) every account.
 */
export type UserSearchVisibility =
...
/**
 * SQL condition: the user holds a role somewhere in `organizationId`'s tree —
 * on the organization itself, one of its offices, or one of their (live)
 * projects.
 */
const isMemberOfOrganizationTree = (organizationId: string): SQL => sql`(
  ${user.id} in (
    ...
    where ${office.organizationId} = ${organizationId}
  ) or ${user.id} in (
...
/**
 * The organization a search scope belongs to, or `null` when the entity does
 * not exist (or the project is soft-deleted).
 */
export const getOrganizationIdForSearchScope = async (
 ...
 *
 * With an organization visibility, fuzzy matches are limited to members of
 * that organization tree; anyone else is returned only on an exact
...
export const searchUsers = async (
  ...
  const matchesVisibleUser =
    "global" in visibility
      ? fuzzyMatch
      : sql`((${fuzzyMatch} AND ${isMemberOfOrganizationTree(visibility.organizationId)}) OR lower(${user.email}) = ${trimmedQuery.toLowerCase()})`;
